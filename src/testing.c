...
extern int rl_vi_bword (int, int);
extern int rl_vi_eword (int, int);
# 288 "/usr/include/readline/readline.h" 3 4
...
# 486 "/usr/include/readline/readline.h" 3 4
...
# 655 "/usr/include/readline/readline.h" 3 4
...
# 711 "/usr/include/readline/readline.h" 3 4
...
# 729 "/usr/include/readline/readline.h" 3 4
...
# 821 "/usr/include/readline/readline.h" 3 4
...
# 878 "/usr/include/readline/readline.h" 3 4
...
# 168 "/usr/include/readline/history.h" 3 4
...
# 222 "/usr/include/readline/history.h" 3 4
...
# 13 "myshell_final.c" 2
# 35 "myshell_final.c"
char command_line[1024];
char *command_tokens[200];
int num_tokens;
...
char *env_variables[100];
char *env_var_values[100];
int env_var_count;
char *prompt_string;
char *pwd;
...
void set_env_variable(char *key, char *value)
{
 ...
 for (i = 0; i < env_var_count; i++) {
  ...
  if (env_variables[i]) {
   ...
   if (strcmp(key, env_variables[i]) == 0) {
    ...
    env_var_values[i] = (char *)malloc(1024);
 ...
 if (env_var_count != 100) {
  i = 0;

  while ((env_variables[i] != ((void *)0)) && (strlen(env_variables[i]) != 0))
   i++;
  env_variables[i] = (char *)malloc(256);
  env_var_values[i] = (char *)malloc(1024);
...
{
 programs[num_progs] = (char *)malloc(64);
 prog_locs[num_progs] = (char *)malloc(512);
...
{
 env_variables[0] = (char *)malloc(256);
 strcpy(env_variables[0], "PS");
 env_var_values[0] = (char *)malloc(1024);
 strcpy(env_var_values[0], "myshell");
 prompt_string = env_var_values[0];

 env_variables[1] = (char *)malloc(256);
 strcpy(env_variables[1], "USER");
 env_var_values[1] = (char *)malloc(1024);
 getlogin_r(env_var_values[1], 1024);

 env_variables[2] = (char *)malloc(256);
 strcpy(env_variables[2], "PWD");
 env_var_values[2] = (char *)malloc(1024);
 getcwd(env_var_values[2], 1024);
 pwd = env_var_values[2];

 env_variables[3] = (char *)malloc(256);
 strcpy(env_variables[3], "TERM");
 env_var_values[3] = (char *)malloc(1024);
 strcpy(env_var_values[3], "xterm");

 env_variables[4] = (char *)malloc(256);
 strcpy(env_variables[4], "HOME");
 env_var_values[4] = (char *)malloc(1024);
...
void init(void)
{
 ...
 for (i = 0; i < 100; i++) {
  programs[i] = (char *)malloc(64);
  prog_locs[i] = (char *)malloc(512);
 }
...
void tokenize_command(void)
{
 ...
 char *acc = (char *)calloc(256, sizeof(char));
 ...
 while (cur) {
  ...
  if (quoted == 1) {
   ...
   if (l > 0) {
    ...
    ++k;
    free(acc);
    acc = (char *)calloc(256, sizeof(char));
    j = 0;
   }
   quoted = 0;
  }
...
int open_next_token(char *filename, int decide)
{
 ...
 case 69:
  ...
  break;
 case 49:
  fd = open(filepath, 01 | 02000);
  break;
 case 42:
  fd = open(filepath, 0100 | 01 | 01000, 0400 | 0200 | (0400 >> 3) | (0200 >> 3) | ((0400 >> 3) >> 3));
  break;
 default:
  fd = -1;
 }
...
# 700 "myshell_final.c"
   ...
   if (success) {
    ...
    child_num--;
    if (((((__extension__ (((union { __typeof(prev_stat) __in; int __i; }) { .__in = (prev_stat) }).__i))) & 0x7f) == 0))
     if (((((__extension__ (((union { __typeof(prev_stat) __in; int __i; }) { .__in = (prev_stat) }).__i))) & 0xff00) >> 8))
      break;
   } else if (failure) {
    if (waitpid(cpid, &prev_stat, 0) == -1) {
 ...
 hack_delay.tv_sec = 0;
 hack_delay.tv_nsec = 5000000;
 nanosleep(&hack_delay, ((void *)0));
}
