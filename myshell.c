...
void set_default_env_vars()
{
	...
	strcpy(env_var_values[4], "/home/rounak");
...
int is_builtin()
{
	if (strcmp("exit", command_tokens[0]) == 0) {
		...
		return 1;
	} else if (strcmp("echo", command_tokens[0]) == 0) {
		execute_command();
		return 1;
		...
		return 1;
	} else if (strcmp("intro", command_tokens[0]) == 0) {
		printf("\tMyShell, ma belle\nThese are words that go together well\n\tMy MyShell\n\n\tMyShell, ma belle\nSont les mots qui vont tres bien ensemble\n\tTres bien ensemble\n");
		return 1;
	}
	return 0;
}
